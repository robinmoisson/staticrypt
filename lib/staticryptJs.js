...
function init(staticryptConfig, templateConfig) {
    ...
    async function decryptAndReplaceHtml(hashedPassword) {
        ...
        // We defer getting the potentially very large encrypted data, and allow
        // decode to create the buffer so it will be able release the reference,
        // making it eligible for GC as soon as it's no longer needed.

        let result = await decode(iv, async () => await getEncrypted(), hmac, hashedPassword, salt);
        if (!result.success) {
            return false;
        }
     ...
     *
     * @returns {Promise<boolean>} true if we decrypted and replaced the whole page, false otherwise
     */
    async function decryptOnLoadFromRememberMe() {
        const { rememberDurationInDays } = staticryptConfig;
        const { rememberExpirationKey, rememberPassphraseKey } = templateConfig;

        // if we are logging out, terminate
        if (logoutIfNeeded()) {
            return false;
        }
