...
/**
 * Translates between Base64 encoded strings
 * and Uint8Array.
 */
const Base64Encoder = isNode
    ? {
          // Node version

          /**
           * Base64 encoded string -> Uint8Array
           * @param {string|null} b64String
           * @returns {Uint8Array|null}
           */
          parse: function (b64String) {
              const bytes = Buffer.from(b64String, "base64");
              return bytes;
          },

          /**
           * Uint8Array -> Base64 encoded string
           * @param {Uint8Array|null} bytes
           * @returns {string|null}
           */
          stringify: function (bytes) {
    ...
    : {
           ...
           * @returns {Uint8Array}
           */
          parse: async function (b64String) {
              if (!b64String) {
                  return null;
              }

              const dataUrl = `data:application/octet-stream;base64,${b64String}`;
          ...
          stringify: function (bytes) {
              ...
              return new Promise((resolve, reject) => {
                  reader.onload = () => {
                      // The result includes "data:;base64," prefix, so we remove it
...
/**
 * Implements compression / decompression of UintArray/ArrayBuffer
 */
const Compressor = isNode
    ? {
          // Node version

          /**
           * Uint8Array -> Compressed Uint8Array
           ...
           * @returns {Uint8Array}
           */
          decompress: async function (compressed) {
              const bytes = new Uint8Array(zlib.gunzipSync(compressed));
    ...
    : {
           ...
           */
          decompress: async function (compressed) {
              const compressedStream = new Blob([compressed]).compressedBlob.stream();
...
function getRandomAlphanum() {
    ...
    do {
        randByte = crypto.getRandomValues(new Uint8Array(1))[0];
    } while (randByte >= 256 - (256 % possibleCharacters.length));

    // Take the modulo of the parsed integer to get a random number between 0 and totalLength - 1
    const randomIndex = randByte % possibleCharacters.length;
